...
- [ ] **Step 5:** Add a `## Phase 26` section to `docs/superpowers/plans/00-roadmap.md` so the roadmap does not end at 23.
- [ ] **Step 6:** Run the drill dry run: all four scenarios in `#test-firedrill` against the deployed Worker (how-to question, small feature request, planted bug, large feature request), plus `pnpm exec tsx scripts/live-drill-readonly.mjs` for the ship path. Record the outcomes in `phase-26-notes.md`.
