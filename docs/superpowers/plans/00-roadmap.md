...
| **[`Zellify/web2app-rebuild`](https://github.com/Zellify/web2app-rebuild)** | **The product monorepo.** Zellify's actual codebase — what customers use and what bugs live in. | The agent clones this in the sandbox, fixes bugs in it, and opens PRs **against its `staging` branch**. Read its root doc first: **`AGENTS.md`** (678 lines, confirmed 2026-08-15), exactly as the brief says. `CLAUDE.md` is a one-line pointer to it. It documents local dev setup, the branch flow, and PR conventions; the PR and issue procedures themselves live in `.agents/skills/m-create-pr/` and `.agents/skills/m-create-linear-task/`. |
...
| **18** | **`Zellify/web2app-rebuild` root `AGENTS.md`** — local dev setup, branch flow, PR conventions | Not an MCP server, but the same class of dependency: the ship loop has to follow conventions we cannot guess. Read 2026-08-15; findings are in the Phase 18 plan's "Ground truth" section and `phase-20-notes.md`. | *(cloned — no MCP needed)* |
...
**Decisions this plan fixed against the old sketch:**
1. **The root doc is `AGENTS.md`** (678 lines); `CLAUDE.md` is a one-line pointer to it. It yields constraints the sketch could not know: `pnpm build-packages` mandatory before first dev, Infisical-wrapped dev scripts, `apps/web` on port 3000, a 6 GB heap for the dashboard, Biome-via-Ultracite, and the `staging` branch flow. *(An earlier revision of this line said `AGENTS.md` did not exist and read the default branch's then-529-line `CLAUDE.md` instead. That was wrong twice over: `AGENTS.md` was already canonical on `staging`, and it has since landed on the default branch too. The version pins taken from that stale file — pnpm 9.12.2 under `engine-strict` — are not current; the repo runs pnpm 11.x with the global virtual store explicitly disabled. Corrected 2026-08-15.)*
