...
> **Status 2026-08-13:** All five steps done and verified live, on deployed version `2201184a-0c68-47db-b971-2abfe68edcdf`; release gate G1 closed. **All three decision types proven in production**, each with an authenticated `PATCH` → `200` and validated identity: **approve** (`apr:b64f4a23…`, run `bca4b327…`) → `blocked`/`identity_unavailable`; **reject** (`apr:16bb9cc7…`, run `f723b51f…`) → `delivery` stays `none`, no send attempted; **edit** (`apr:0ab09578…`, same run) → `blocked`, with the human's text (not the model's draft) handed to the sender. Both halves of the final review's Important 1 are proven live with measured character offsets, and the edit's episode truncated at exactly 1,000 chars with both texts surviving — closing the truncation gap too. `escalated` 0 → 3; open queue empty. **Not exercised live:** a `memory.recall` surfacing these lessons (pending Zep extraction; extraction itself is proven live), plus the CAS loser, viewer/outsider 403, invalid-JWT 401 (unreachable from outside), withdraw races and shadow suppression. Full evidence and a closing summary table in `phase-11-notes.md` → "Task 10".
...
- [x] The dashboard API approves, edits, and rejects with validated identity; an edit stores the human's text; a rejection's reason reaches memory. — all three decision types verified live with validated identity (`decidedBy: sayandeten@gmail.com`): approve `apr:b64f4a23…`, reject `apr:16bb9cc7…`, edit `apr:0ab09578…`. The edit stored `edited_text` in D1 and the human's text — not the model's draft — is what `outboundText()` handed the sender (`delivery_error=identity_unavailable` proves the sender was called, not skipped). The rejection's reason *and* rejected draft reached the projected memory episode.
