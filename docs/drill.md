...
- **Slack.** The agent replies in the `#test-firedrill` thread under a connected fire-fighter's
...
| A direct question whose answer we already know triages `wake: 0` — live on 2026-08-16, *"Do you support stripe?"* twice, each a fresh top-level message, `why`: "obvious answer, doesn't require action — Stripe is explicitly listed as supported"; the customer got silence | Recall handed triage the answer and Haiku read *we know it* as *nobody needs to reply*. The Dodo question minutes earlier (unknown to memory) woke fine — the better memory gets, the more direct questions this drops | Fixed prompt-side (`src/triage/prompt.ts`: a direct question wakes even when the answer is known; handled only if a reply is visible in-thread), pinned in `test/triage-prompt.test.ts`. If it recurs, read `triage_decisions.why` for "already"/"obvious" before touching anything else. |
| A run wakes, works out the answer, and its summary says *"`slack.reply` refused"* — no effect row, no approval, nothing posted; approvals come out `blocked: not connected` and the nudge lands in the fallback channel, all with a connected fire-fighter sitting right there. Live on 2026-08-17 10:19 UTC (`7edf4402`) | The old three-day rotation handed the on-duty seat to `ronit@zellify.app` at 00:00 UTC; only `sayandeten@gmail.com` had connected Slack, so `actor` resolved null and every customer-facing write refused `identity_unavailable` — correctly, for the wrong reason | **Fixed by removing the rotation** (`src/identity/speaker.ts`, same day): any roster fire-fighter who has connected can speak — the approver when they clicked, else the first in roster order. If it recurs, `GET /api/roster` from a logged-in tab shows `speaker: null` when nobody has connected; the fix is a fire-fighter clicking "connect Slack" on the dashboard, not a code change. |
