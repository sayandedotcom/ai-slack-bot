...
| **Durable Objects** | One `RunDO` per run, keyed `slack:{channel}:{thread_ts}` or `chat:{uuid}`. Holds the session in its own SQLite, streams over WebSocket, and hibernates at zero cost while an approval sits pending. Phase 25 added a second class, `RunAgent extends Think<Env>` (Agents SDK), bound as `RUN_AGENTS` and selected by the `RUN_CHASSIS` var — deployed **inactive** (`legacy`) behind the strangler flag; see README §*Two run chassis*. | 08, 25 |
...
| Rejected | Why |
|---|---|
| **Vercel + Next 16** *(rejected as a SECOND ORIGIN; adopted 2026-08-27 as a second front-end, live 2026-08-28)* | The sanctioned combo, and permitted rather than mandated. But every pixel is live socket state, so SSR earns nothing; two origins means cross-origin WebSocket auth against Access and a second deploy target. **Single origin collapses all of it, and it still does — the adoption did not abandon that decision, it kept it on a different hostname.** `apps/web` is a Next 16 App Router app rendering the same three surfaces, deployed to Vercel and served at `firefighter.sayande.xyz`: a Cloudflare-proxied CNAME, added to the SAME Access application, with one Worker route (`firefighter.sayande.xyz/api/*`) putting the app and the API back on one origin. So Access issues one cookie, the run socket is first-party, and no CORS exists anywhere. The Worker also keeps serving `apps/dashboard` from its own workers.dev origin and `pnpm run deploy` is unchanged, so there are two front-ends against one API — a transitional state, not a permanent one. `apps/web/BACKEND-GAPS.md` §1 records the problem and the options not taken; `docs/deploy-vercel.md` is the wiring. |
...
| Vercel | $0 — Hobby. It serves the app shell and static assets; every data request is a Worker route on the same hostname and never reaches Vercel. |
| **Total** | **~$70 – 160** |
