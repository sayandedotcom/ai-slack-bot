...
- `.claude/worktrees/phase-24` is a git worktree on branch `worktree-phase-24` (dashboard narration work) that may be ahead of `main`. `.claude/worktrees/think-chassis` (branch `worktree-think-chassis`, locked) is where Phase 25 was built; it has been merged into `main` and is at the same commit.
- Untracked root notes (`TEST-FINDINGS.md`, `LOOM-DEMO-SCENARIOS.md`, `authenticated-proofs.md`, the `*-explainer.md` files) are deliberately untracked — read them, never `git add` them.
