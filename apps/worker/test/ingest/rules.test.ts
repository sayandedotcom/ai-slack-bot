...
describe("classify", () => {
  ...
  it("drops direct messages", () => {
    ...
    expect(classify({ ...base, channel_type: "mpim" }, true)).toBe(
      "dropped_dm"
    );
  });
  ...
  it("drops messages from bots", () => {
    ...
    expect(classify({ ...base, subtype: "bot_message" }, true)).toBe(
      "dropped_bot"
    );
  });

  it("drops join and leave noise", () => {
    expect(classify({ ...base, subtype: "channel_join" }, true)).toBe(
      "dropped_subtype"
    );
    expect(classify({ ...base, subtype: "channel_leave" }, true)).toBe(
      "dropped_subtype"
    );
  });

  it("drops edits and deletions", () => {
    expect(classify({ ...base, subtype: "message_changed" }, true)).toBe(
      "dropped_subtype"
    );
    expect(classify({ ...base, subtype: "message_deleted" }, true)).toBe(
      "dropped_subtype"
    );
  });
  ...
  it("still drops bots and noise in unmapped channels", () => {
    ...
    expect(classify({ ...base, subtype: "channel_join" }, false)).toBe(
      "dropped_subtype"
    );
  });
...
describe("classify self-posts", () => {
  ...
  it("still drops our own bot-token posts — the nudge must never ingest", () => {
    expect(classify({ ...selfPost, user: "U_BOT" }, true, self)).toBe(
      "dropped_bot"
    );
  });

  it("still drops a foreign app's post even when it names a human user", () => {
    expect(classify({ ...selfPost, app_id: "A_THEIRS" }, true, self)).toBe(
      "dropped_bot"
    );
  });

  it("still drops a bot_id post with no user at all", () => {
    expect(classify({ ...selfPost, user: undefined }, true, self)).toBe(
      "dropped_bot"
    );
  });

  it("still drops subtype bot_message unconditionally", () => {
    expect(classify({ ...selfPost, subtype: "bot_message" }, true, self)).toBe(
      "dropped_bot"
    );
  });
  ...
  it("a self-post DM is still a DM", () => {
    expect(classify({ ...selfPost, channel_type: "im" }, true, self)).toBe(
      "dropped_dm"
    );
  });
  ...
  it("a self-post edit is still noise", () => {
    expect(
      classify({ ...selfPost, subtype: "message_changed" }, true, self)
    ).toBe("dropped_subtype");
  });
