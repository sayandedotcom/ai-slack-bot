...
import { makeGuardedExecutor, TRUNCATION_MARK } from "../src/capabilities/executor";
...
function executor(overrides: Partial<typeof PRODUCTION_LIMITS> = {}) {
  ...
  return makeGuardedExecutor(guardLoader(env.LOADER, limits), limits, () => Date.now());
}
...
describe("guarded executor", () => {
  ...
  it("refuses code over the character ceiling without loading an isolate", async () => {
    const out = await executor({ maxCodeChars: 100 }).execute("x".repeat(101), {});
  ...
  it("rejects get() on the guarded loader", () => {
    ...
    expect(() =>
      guardLoader(env.LOADER, PRODUCTION_LIMITS).get("x", async () => ({}) as never),
    ).toThrow();
  });
...
describe("toSafeJson", () => {
  it("passes plain JSON data through", () => {
    expect(toSafeJson({ a: 1, b: ["x"] }, PRODUCTION_LIMITS)).toEqual({ a: 1, b: ["x"] });
  });

  it("refuses a class instance rather than coercing it to {}", () => {
    // Quietly turning an Error into {} produces a result the model reasons
    // about incorrectly.
    expect(() => toSafeJson(new Error("boom"), PRODUCTION_LIMITS)).toThrow(/not plain JSON data/);
  });
  ...
  it("never invokes a hostile toJSON()", () => {
    let called = false;
    const hostile = { toJSON: () => { called = true; return "safe"; } };
  ...
  it("drops __proto__ instead of assigning it", () => {
    ...
    const parsed = JSON.parse('{"__proto__": {"polluted": true}, "ok": 1}') as unknown;
    const safe = toSafeJson(parsed, PRODUCTION_LIMITS) as Record<string, unknown>;
  ...
  it("refuses a non-finite number", () => {
    expect(() => toSafeJson({ n: Number.NaN }, PRODUCTION_LIMITS)).toThrow(/no JSON representation/);
  });

  it("refuses a result over the character cap", () => {
    expect(() => toSafeJson({ s: "x".repeat(500) }, { ...PRODUCTION_LIMITS, maxResultChars: 100 }))
