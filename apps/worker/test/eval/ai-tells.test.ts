...
describe("detectAiTells", () => {
  it("returns [] for the canonical clean reply", () => {
    ...
    expect(detectAiTells(VOICE_EXAMPLES[0]!.good)).toEqual([]);
  });

  describe("preamble", () => {
    it("flags a fixed greeting-opener", () => {
      expect(
        detectAiTells("Thanks for reaching out! Here is the fix.")
      ).toContain("preamble");
      expect(detectAiTells("I'd be happy to help with that.")).toContain(
        "preamble"
      );
    });
    ...
    it("does not flag a reply that merely starts with the word Thanks before substance", () => {
      ...
      expect(
        detectAiTells("Thanks, that matches what we saw in the logs too.")
  ...
  describe("great_question", () => {
    it("flags case-insensitively", () => {
      expect(detectAiTells("Great question! Let's dig in.")).toContain(
        "great_question"
      );
      expect(detectAiTells("good question, let me check.")).toContain(
        "great_question"
      );
    });
    ...
    it("does not flag unrelated use of the word question", () => {
      expect(
        detectAiTells("That's a fair question about billing.")
      ).not.toContain("great_question");
    });
  });
  ...
  describe("bulleted_recap", () => {
    ...
    it("does not flag a summary sentence with no bullet list", () => {
      expect(detectAiTells("To summarize, the fix is out.")).not.toContain(
        "bulleted_recap"
      );
    });
  });
  ...
  describe("closing_restatement", () => {
    it("flags a fixed closer", () => {
      expect(
        detectAiTells(
          "The fix is out. Let me know if you have any other questions."
        )
      ).toContain("closing_restatement");
      expect(detectAiTells("Fixed now. Hope this helps.")).toContain(
        "closing_restatement"
      );
    });
    ...
    it("does not flag an unrelated closing sentence", () => {
      expect(
        detectAiTells("Fixed now. I'll post here if it regresses.")
      ).not.toContain("closing_restatement");
    });
  });

  describe("exclaimed_thanks", () => {
    it("flags exclamation-marked gratitude", () => {
      expect(detectAiTells("Thanks for flagging!")).toContain(
        "exclaimed_thanks"
      );
    });

    it("does not flag plain gratitude without an exclamation mark", () => {
      expect(detectAiTells("Thanks for flagging this.")).not.toContain(
        "exclaimed_thanks"
      );
    });
  });
  ...
  describe("typography tells (policy.ts, 2026-08-14)", () => {
    it("flags an em dash", () => {
      expect(detectAiTells("Fixed now — deployed at 04:12.")).toContain(
        "em_dash"
      );
    });
    ...
    it("does not flag a hyphen or a plain comma", () => {
      expect(
        detectAiTells("Fixed now, deployed at 04:12 as a one-off.")
    ...
    it("does not flag a colon", () => {
      expect(detectAiTells("Two things: exports, imports.")).not.toContain(
        "semicolon"
      );
    });
    ...
    it("does not flag plain ASCII text", () => {
      expect(detectAiTells("Fixed now, deployed at 04:12.")).not.toContain(
        "emoji"
      );
    });
  ...
  describe("code spans are exempt from the typography rules", () => {
    it("does not flag a semicolon inside an inline code span", () => {
      expect(
        detectAiTells(
          "Run `git log --oneline; git status` and paste the output."
        )
    ...
    it("does not flag an exclamation mark inside an inline code span", () => {
      expect(
        detectAiTells("The guard is `a !== b`, so the branch never runs.")
    ...
    it("still flags the same character in prose alongside a clean code span", () => {
      expect(
        detectAiTells("Fixed now; run `SELECT 1; SELECT 2;` to confirm.")
      ).toContain("semicolon");
    });
  ...
  it("reports multiple tells in one text", () => {
    const text =
      "Great question! Thanks for flagging! Let me know if you have any other questions.";
