...
function get(path: string, headers: Record<string, string> = {}, envOverrides = {}) {
  return slackOAuth.request(`${ORIGIN}${path}`, { headers }, testEnv(envOverrides));
}
...
describe("GET /oauth/slack/start", () => {
  ...
  it("403s a viewer", async () => {
    const res = await get("/oauth/slack/start", { "Cf-Access-Jwt-Assertion": VIEWER });
  ...
  it("503s by VARIABLE NAME when IDENTITY_KEY is unset, never a value", async () => {
    const res = await get("/oauth/slack/start", { "Cf-Access-Jwt-Assertion": FIREFIGHTER }, { IDENTITY_KEY: undefined });
  ...
  it("redirects a fire-fighter to Slack with the user scope, derived redirect_uri, and a verifiable state", async () => {
    const res = await get("/oauth/slack/start", { "Cf-Access-Jwt-Assertion": FIREFIGHTER });
    ...
    expect(await verifyState(key, state, "slack", Date.now())).toEqual({ email: FIREFIGHTER });
  });
});
...
async function goodState(email = FIREFIGHTER): Promise<string> {
  return mintState(await importStateKey(IDENTITY_KEY), email, "slack", Date.now());
}
...
describe("GET /oauth/slack/callback", () => {
  it("403s a forged state and makes NO exchange call", async () => {
    stubSlack(okPayload);
    const res = await get("/oauth/slack/callback?code=abc&state=forged.nonsense");
    expect(res.status).toBe(403);
  ...
  it("403s a state minted for github, and makes NO exchange call", async () => {
    stubSlack(okPayload);
    const foreign = await mintState(await importStateKey(IDENTITY_KEY), FIREFIGHTER, "github", Date.now());
    const res = await get(`/oauth/slack/callback?code=abc&state=${encodeURIComponent(foreign)}`);
    expect(res.status).toBe(403);
    expect(calls).toEqual([]);
  });
  ...
  it("403s an expired state, and makes NO exchange call", async () => {
    ...
    const res = await get(`/oauth/slack/callback?code=abc&state=${encodeURIComponent(stale)}`);
    expect(res.status).toBe(403);
    expect(calls).toEqual([]);
  });
  ...
  it("exchanges the code form-encoded and stores sealed ciphertext, then 302s home", async () => {
    ...
    const res = await get(`/oauth/slack/callback?code=thecode&state=${encodeURIComponent(state)}`);
  ...
  it("503s on a missing client secret before touching Slack", async () => {
    ...
    expect(res.status).toBe(503);
    expect((await res.json<{ message: string }>()).message).toBe("missing configuration: SLACK_CLIENT_SECRET");
    expect(calls).toEqual([]);
  });
});
