...
function memoryStore(
  facts: { factId: string; fact: string; episodeUuids?: string[] }[] = []
): MemoryStore {
  return {
...
describe("memory.findCustomers — invariant 36", () => {
  it("is refused on a Slack-origin run, which is already scoped", async () => {
    ...
    await expect(
      makeMemoryTools(ctx).findCustomers.run({ query: "pulse" })
    ).rejects.toThrow();
  });
...
describe("memory.recall — graph scoping", () => {
  it("never interpolates model input into a graph id", async () => {
    ...
    await expect(
      makeMemoryTools(ctx).recall.run({
        query: "checkout",
  ...
  it("uses a reference minted in this execution", async () => {
    ...
    const [match] = (await tools.findCustomers.run({ query: slug })) as {
      customerRef: string;
    }[];
...
describe("memory.cite", () => {
  it("accepts only fact ids recalled in this same execution", async () => {
    ...
    await expect(
      makeMemoryTools(ctx).cite.run({ factIds: ["never-recalled"] })
    ).rejects.toThrow();
  });
});
