...
import { createOrGetRun, getRunById, readRunUsage, setRunStatus } from "../src/run/repository";
...
describe("status projection", () => {
  it("applies a legal transition", async () => {
    ...
    expect(await projectStatus(env.DB, run.id, "live", Date.now())).toEqual({ applied: true });
  ...
  it("is idempotent on the same status and writes nothing", async () => {
    ...
    const outcome = await projectStatus(env.DB, run.id, "live", (before?.updatedAt ?? 0) + 5_000);
    ...
    expect((await getRunById(env.DB, run.id))?.updatedAt).toBe(before?.updatedAt);
  });
  ...
  it("loses cleanly when another projection moved the row first", async () => {
    ...
    expect(["done", "failed"]).toContain((await getRunById(env.DB, run.id))?.status);
  });
  ...
  it("does not fail on a missing row", async () => {
    ...
    expect(await projectStatus(env.DB, crypto.randomUUID(), "live", Date.now())).toEqual({
      applied: false,
...
describe("usage rows", () => {
  ...
  it("leaves one row when the same step is recorded twice", async () => {
    ...
    await recordUsage(env.DB, { ...row, providerRequestId: "req_2", costNanoUsd: 9_000_000 });
  ...
  it("stores cost as an integer", async () => {
    const run = await freshRun();
    await recordUsage(env.DB, usageRow({ runId: run.id, costNanoUsd: 1_234.9, latencyMs: 10.4 }));
  ...
  it("keys a row on the generation, attempt and step and nothing else", () => {
    expect(usageRowId({ generationId: "gen-9", attempt: 2, stepIndex: 5 })).toBe(
      "usage:gen-9:2:5",
    );
  });
});
