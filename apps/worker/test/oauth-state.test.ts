...
const SECRET = btoa(String.fromCharCode(...new Uint8Array(32).map((_, i) => (i * 7 + 3) & 0xff)));
...
describe("mintState / verifyState round trip", () => {
  it("returns the email it was minted with", async () => {
    ...
    expect(await verifyState(k, state, "slack", NOW + 1000)).toEqual({ email: "ronit@zellify.app" });
  });
...
describe("verifyState refuses everything else", () => {
  it("expires ten minutes out — good at 9, null at 11", async () => {
    const k = await key();
    const state = await mintState(k, "ronit@zellify.app", "slack", NOW);
    expect(await verifyState(k, state, "slack", NOW + 9 * 60_000)).not.toBeNull();
  ...
  it("rejects a payload swapped for another identity's, signature kept", async () => {
    ...
    const forged = base64UrlEncode(
      JSON.stringify({ e: "attacker@example.com", p: "slack", x: NOW + 600_000, n: "AAAA" }),
    );
    expect(await verifyState(k, `${forged}.${mine.split(".")[1]}`, "slack", NOW)).toBeNull();
  });
  ...
  it("rejects a signature borrowed from a different state", async () => {
    ...
    expect(await verifyState(k, `${mine.split(".")[0]}.${other.split(".")[1]}`, "slack", NOW)).toBeNull();
  });

  it("rejects a state signed with a different key", async () => {
    const foreign = await importStateKey(btoa(String.fromCharCode(...new Uint8Array(32).fill(9))));
  ...
  it("rejects a slack state replayed at the github callback", async () => {
    ...
    expect(await verifyState(k, state, "slack", NOW)).toEqual({ email: "ronit@zellify.app" });
  });
