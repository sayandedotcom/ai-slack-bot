...
if (!KEY && !DRY) {
  console.error(
    "LANGSMITH_API_KEY is not set (put it in the environment for this one command; it is not read from .dev.vars on purpose)."
  );
  process.exit(2);
}
...
const TRACES = [
  trace({
    ...
    outputs: {
      bundle_id: "app.sidehop.ios",
      display_name: "Sidehop",
    ...
    children: [
      toolCall(
        "fetch_site_manifest",
        { url: "https://sidehop.app" },
        {
          title: "Sidehop — earn on the side",
          theme_color: "#0F766E",
          icons: 3,
        },
        820
      ...
      llmCall(
        "claude-haiku-4-5",
        "Derive an iOS app config from this manifest…",
        '{"bundle_id":"app.sidehop.ios","display_name":"Sidehop","primary_color":"#0F766E","tabs":4}',
        1430
      ),
    ],
  }),
  trace({
    ...
    children: [
      ...
      llmCall(
        "claude-haiku-4-5",
        "Derive an Android app config from this manifest…",
        '{"bundle_id":"app.sidehop.android","display_name":"Sidehop","primary_color":"#0F766E","tabs":4}',
        1210
      ),
    ],
  }),
  ...
  trace({
    ...
    outputs: {
      reply:
        "Great news — your Android app went live on the Play Store on Tuesday! 🎉",
    },
    metadata: { channel: "in-app-chat" },
    children: [
      toolCall(
        "lookup_app_status",
        { customer_slug: "sidehop", platform: "android" },
        { status: "broken", store_status: "in_review", last_build: "failed" },
        210
      ),
      llmCall(
        "claude-haiku-4-5",
        "Customer asks: Is my Android app live on the Play Store yet? Tool result: {status: broken, store_status: in_review, last_build: failed}. Reply helpfully.",
        "Great news — your Android app went live on the Play Store on Tuesday! 🎉",
        980
      ),
    ],
  ...
  trace({
    ...
    children: [
      toolCall(
        "fetch_site_manifest",
        { url: "https://firedrill.example" },
        undefined,
        30_000,
        {
          error:
            "TimeoutError: fetching https://firedrill.example took longer than 30000ms",
        }
      ),
    ],
  }),
  trace({
    ...
    outputs: {
      icon_url: "https://assets.example/firedrill/icon-1024.png",
      sizes: [1024, 180, 120],
    },
    children: [
      llmCall(
        "claude-haiku-4-5",
        "Describe a flat app icon for 'Firedrill Demo' in #DC2626…",
        "A white flame silhouette centered on a #DC2626 rounded square.",
        640
      ),
      toolCall(
        "render_icon",
        { prompt: "white flame on #DC2626", size: 1024 },
        { url: "https://assets.example/firedrill/icon-1024.png" },
        2100
      ),
    ],
  }),
...
if (DRY) {
  console.log(
    JSON.stringify(
      { project: PROJECT, runs: post.length, sample: post[0] },
      null,
      2
    )
  );
...
console.log(
  `ingested ${post.length} runs across ${TRACES.length} traces into project "${PROJECT}" (HTTP ${res.status})`
);

// Ingestion is asynchronous; poll until the root runs are queryable.
const sessions = await (
  await fetch(`${ENDPOINT}/sessions?name=${encodeURIComponent(PROJECT)}`, {
    headers,
  })
).json();
...
if (!project) {
  console.error(
    "project not visible yet — re-run the lookup in a minute: GET /sessions?name=" +
      PROJECT
  );
  process.exit(1);
}
...
for (let attempt = 0; attempt < 12 && roots < TRACES.length; attempt++) {
  ...
  const q = await fetch(`${ENDPOINT}/runs/query`, {
    method: "POST",
    headers,
    body: JSON.stringify({ session: [project.id], is_root: true, limit: 50 }),
  });
...
const ws = await (await fetch(`${ENDPOINT}/workspaces`, { headers }))
  .json()
  .catch(() => null);
