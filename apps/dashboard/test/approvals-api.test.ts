...
import { decide, fetchApproval, fetchOpenApprovals } from "../src/approvals/api";
...
describe("fetchOpenApprovals", () => {
  it("parses a 200 list and unwraps `approvals`", async () => {
    const fetchSpy = stubFetch(() => jsonResponse(200, { approvals: [SUMMARY] }));

    await expect(fetchOpenApprovals()).resolves.toEqual([SUMMARY]);
    expect(String(fetchSpy.mock.calls[0]?.[0])).toBe("/api/approvals?state=open");
  });

  it("throws an ApiError naming only the path on a failure", async () => {
    stubFetch(() => new Response(BODY, { status: 500 }));

    const error = (await fetchOpenApprovals().catch((e: unknown) => e)) as ApiError;
...
describe("decide", () => {
  it("PATCHes the action body and reports `decided` from the returned card", async () => {
    const fetchSpy = stubFetch(() =>
      jsonResponse(200, { approval: { ...CARD, decision: "edited" }, resolutionDelivered: true }),
    );

    await expect(decide("ap_1", { action: "edit", text: "restart it gently" })).resolves.toEqual({
      result: "decided",
      decision: "edited",
    });
    ...
    expect(JSON.parse(String(init.body))).toEqual({ action: "edit", text: "restart it gently" });
  });
  ...
  it("returns an ApiError result on a 422 instead of throwing", async () => {
    stubFetch(() => jsonResponse(422, { code: "invalid_action", message: BODY }));
  ...
  it("maps a 404 and a network throw to error results with no body text", async () => {
    stubFetch(() => jsonResponse(404, { code: "unknown_approval", message: BODY }));
