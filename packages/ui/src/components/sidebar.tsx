...
function SidebarProvider({
  ...
  //
  // `setOpenMobile` is NOT a dependency: it is a useState setter, stable for
  // the life of the component by React's own contract, so listing it only
  // tells a reader to wonder whether it can change. `setOpen` stays, because
  // it is the local useCallback above and genuinely does change.
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    // `setOpenMobile` omitted for the same reason as above: a useState setter
    // is stable, so it can never be the thing that invalidates this memo.
